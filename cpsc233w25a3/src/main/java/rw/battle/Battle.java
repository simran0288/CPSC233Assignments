...
public class Battle {
        ...
        for (Robot robot : robots) {
            ...
            if (robot.isAlive()) {
                ...
                if (attackWhere == null) {
                    ...
                    if (moveWhere == Direction.STAY || canMoveOnTopOf(row, column, moveWhere)) {
                    ...
                    if (canBeAttacked(row, column, attackWhere)) {
                        if (entity instanceof Robot attacked) {
    ...
    /**
     * Remove non-robot by row and column
     *
     * @param row    Row to remove at
     * @param column Column to remove at
     */
    public void removeNonRobot(int row, int column) {
        if (this.battle[row][column] instanceof Robot) {
            throw new RuntimeException("There is a robot at this location, use removeRobot(Entity) or removeRobot(row, column)!");
        }
        this.battle[row][column] = null;
    }

    /**
     * Remove robot by row and column
     *
     * @param row    Row to remove at
     * @param column Column to remove at
     */
    public void removeRobot(int row, int column) {
        if (this.battle[row][column] instanceof Robot robot) {
            this.battle[row][column] = null;
            this.robots.remove(robot);
            this.locations.remove(robot);
        } else {
            throw new RuntimeException("There is a robot at this location, use removeNonRobot(row, column)!");
        }
    }

    /**
     * Remove robot by object (lookup location and then remove at location)
     *
     * @param robot Robot entity to remove
     */
    public void removeRobot(Entity robot) {
        if (robot instanceof Robot) {
            Integer[] location = locations.get(robot);
            removeRobot(location[0], location[1]);
        } else {
            throw new RuntimeException("There is a robot at this location, use removeNonRobot(row, column)!");
        }
    }
    ...
    public boolean invalid(int row, int column) {
        return row < 0 || row >= battle.length || column < 0 || column >= battle[0].length;
    }
